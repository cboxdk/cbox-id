...
use App\Http\Middleware\QueueMonitorContentSecurityPolicy;
...
/*
|--------------------------------------------------------------------------
| Queue monitor — cboxdk/laravel-queue-monitor
|--------------------------------------------------------------------------
|
| PUBLISHED RATHER THAN INHERITED, because three of the package defaults are wrong
| for an identity provider:
|
|  - It would store every job's serialized payload. Ours carry webhook bodies and,
|    in whatever job somebody adds next, tokens and email addresses. Off, and
|    enforced beyond this file — see App\Providers\QueueServiceProvider.
|  - Its dashboard would answer at /queue-monitor on EVERY host, customer domains
|    included, behind nothing but an APP_ENV check. It lives under /platform/queues
|    here, for platform operators, on the platform root's host only.
|  - Its REST API would be a second, token-less door. Off; and if it is ever turned
|    on, it sits behind the same operator session as the dashboard.
|
| The console links it from Platform › Queues. See docs/operations/queue-workers.md.
|
*/
...
return [
    ...
    */
    'storage' => [
        // NEVER, and not an environment variable: a flag an .env can flip is a flag that
        // gets flipped while debugging and forgotten. Replay needs the payload, so replay
        // is off too — Laravel's `failed_jobs` table keeps what `queue:retry` needs.
        'store_payload' => false,
    ...
    */
    'retention' => [
        // A week of history: long enough to look back across a weekend incident, short
        // enough that failure messages naming customers do not accumulate.
        'days' => 7,
        ...
        // A second bound for a busy week: the jobs table lives in the application's own
        // database, and a monitor must not become its largest table.
        'max_rows' => env('QUEUE_MONITOR_MAX_ROWS', 100_000),

        // Which statuses to prune (empty array = prune all statuses)
        // Failed and timeout jobs are included to prevent unbounded growth
        // EVERY status. The package default leaves `queued` and `processing` rows forever
        // — and weeks with no worker leave exactly those behind.
        'prune_statuses' => [],
        ...
        // Every job here times out at 75 s (App\Platform\Queues\WorkerProfile), so a row
        // still `processing` after 15 minutes is a worker that was killed mid-job.
        'resolve_stuck_after_minutes' => env('QUEUE_MONITOR_RESOLVE_STUCK_AFTER_MINUTES', 15),
    ...
    */
    'api' => [
        // OFF. The dashboard does not use it (it calls its own routes under the UI prefix),
        // so it would only be a second way in. Turned on, it is the SAME door as the
        // dashboard — the operator's browser session, on the platform root's host — never
        // the package's bare `api` group, which authenticates nobody.
        'enabled' => (bool) env('QUEUE_MONITOR_API_ENABLED', false),
        'prefix' => 'platform/queues/api',
        'middleware' => ['web', 'plane:operator', 'platform.auth:optional', AuthenticateOperator::class],
    ...
    */
    'ui' => [
        ...
        // ONE URL, beneath the Platform section it is linked from. The package default
        // (`/queue-monitor`) is not registered at all.
        'route_prefix' => 'platform/queues/monitor',
        // The same locks as every Platform page, plus the host bulkhead those pages do not
        // need and this one does (see App\Platform\Queues\QueueMonitorAccess), plus the
        // one CSP exception the dashboard's Alpine build requires — confined to these
        // routes (see QueueMonitorContentSecurityPolicy).
        'middleware' => ['web', 'plane:operator', 'platform.auth:optional', AuthenticateOperator::class, QueueMonitorContentSecurityPolicy::class],
        'per_page' => 35,
