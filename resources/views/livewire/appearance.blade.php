...
/**
 * Appearance — the hosted sign-in Theme Editor. Editing and preview are entirely
 * client-side (Alpine `themeEditor`, in bundled app.js) for zero latency; this
 * component only seeds the initial state and persists on save. Everything the client
 * sends is re-sanitized through {@see Appearance::fromArray} before it touches
 * settings, so a malformed payload degrades to the preset default, never an error.
 */
new #[Layout('components.layouts.app', ['title' => 'Appearance'])] class extends Component
{
    ...
    public function save(array $theme, Organizations $organizations): void
    {
        ...
        // Logo is stored alongside the palette (kept out of the appearance block so the
        // existing brand_logo_url pipeline keeps working). https only.
        $logo = is_string($theme['logo'] ?? null) ? trim($theme['logo']) : '';
        $logo = ($logo !== '' && filter_var($logo, FILTER_VALIDATE_URL) !== false && str_starts_with($logo, 'https://'))
            ? $logo : null;
        ...
        $organizations->updateSettings($orgId, [
            ...
            // Keep the legacy single-colour brand in sync for anything still reading it.
    ...
    public function with(): array
    {
        return [
            ...
            'radii' => ThemePresets::RADII,
        ];
    }
}; ?>

<div x-data="themeEditor(@js($appearance), @js($presets), @js($fonts), @js($radii))" x-cloak>
    <x-page-header title="Appearance" subtitle="Theme the hosted sign-in your users see — presets, colours, corners, type. Changes preview live and apply to every sign-in page for your organization.">
        <x-slot:actions>
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,360px)_1fr] items-start">
        ...
        <div class="space-y-5 lg:sticky lg:top-6">
            ...
            <section class="card p-4">
                ...
                {{-- Live AA readout — primary on background --}}
            ...
            <section class="card p-4 space-y-4">
                ...
                <div>
                    <label class="label" for="logo">Logo URL <span style="color:var(--faint)">(https, optional)</span></label>
                    <input id="logo" type="url" x-model="draft.logo" class="input" placeholder="https://acme.com/logo.svg" spellcheck="false">
                </div>
            </section>
        ...
        <div class="lg:sticky lg:top-6">
            ...
            <p class="mt-3 text-[12px]" style="color:var(--faint)">This is exactly how your sign-in renders — the preview shares the resolver that themes the live page.</p>
        </div>
    </div>
</div>
