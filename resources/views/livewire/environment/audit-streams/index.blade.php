...
<div>
    ...
    <div class="mt-6">
        <input wire:model.live.debounce.300ms="search" type="search" class="input" style="max-width:24rem" placeholder="Search by name" aria-label="Search log streams">
        {{-- SC 4.1.3: the list re-renders on a debounced keystroke with no focus
             ...
             narrowed to nothing. --}}
        <p role="status" aria-live="polite" class="sr-only">{{ $streams->count() }} {{ \Illuminate\Support\Str::plural('stream', $streams->count()) }} found.</p>
    </div>
