<x-mail.layout>
    <h1 style="margin:0 0 12px;font-size:20px;color:#14161c">Help run {{ $organization }} on {{ $brand }}</h1>
    <p style="margin:0 0 20px;color:#5b616e;font-size:15px;line-height:1.6">
        <b>{{ $inviter }}</b> invited you to administer <b>{{ $organization }}</b>@if ($role) as <b>{{ $role }}</b>@endif on {{ $brand }} —
        the console for its identity providers: environments, members and billing. Accept to set a password and sign in.
    </p>
