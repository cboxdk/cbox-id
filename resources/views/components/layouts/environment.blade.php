        ...
        <header class="hidden lg:flex cbx-topbar items-center justify-between">
            <nav class="flex items-center gap-1.5 text-[13px] min-w-0" aria-label="Breadcrumb">
                ...
                {{-- Acting organization. Every page below is scoped to it, so it belongs
                     ...
                     A Livewire component rather than markup here, because it SEARCHES: an
                     environment's organizations are unbounded, and enumerating them into
                     the chrome sent a 3.5 MB document on every page to any customer with
                     a few thousand. See the component for that account. --}}
                <span style="color:var(--faint)" aria-hidden="true">/</span>
                <livewire:console.organization-switcher />
            </nav>
